...
export async function POST(req: Request) {
  ...
  try {
    const { mediaType, data, fileName, activeAccountId, uploadedBy } = (await req.json()) as {
      mediaType: string;
      data: string;
      fileName?: string;
    ...
    if (!activeAccountId || typeof activeAccountId !== "string") {
      return NextResponse.json(
        { error: "activeAccountId_required", message: "activeAccountId is required and must be a string" },
        { status: 400 },
      );
    }
